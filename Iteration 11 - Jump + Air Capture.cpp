...
bool isAirborneAt(int row, int col, const vector<Airborne>& airborne) {
...
bool hasPendingMoveFrom(int row, int col, const vector<PendingMove>& pending) {
...
optional<Airborne> getAirborneAt(int row, int col, const vector<Airborne>& airborne) {
    for (const auto& air : airborne) {
        if (air.row == row && air.col == col) return air;
    }
    return nullopt;
}
...
int runBoardCommands() {
    ...
    vector<Airborne> airborne;
    long long clockMs = 0;
    const long long MOVE_TIME = 1000;
    const long long JUMP_TIME = 1000;
            ...
            if (isPiece(clicked)) {
                if (!selected.has_value()) {
                    ...
                    if (selected->row == row && selected->col == col) {
                        if (!hasPendingMoveFrom(row, col, pending) && !isAirborneAt(row, col, airborne)) {
                            airborne.push_back(Airborne{row, col, selTok, clockMs + JUMP_TIME});
                        }
                        selected.reset();
                    } else if (sameColor(selTok, clicked)) {
                        selected = Pos{row, col};
